...
export const createPaymentSession = onCall(
  ...
  async (request) => {
    ...
    try {
      ...
      logger.info('Transaction créée', { txId: transactionRef.id });
...
export const onPaymentSuccess = onDocumentWritten(
  ...
  async (event: any) => {
    ...
    try {
      ...
      logger.info('SMS confirmation à envoyer', { phone: buyerPhone, ticketIds });
      ...
      logger.info('Paiement traité avec succès', { txId, ticketIds });
    } catch (error) {
...
export const onPaymentFailed = onDocumentWritten(
  ...
  async (event) => {
    ...
    logger.warn('Paiement échoué', { txId: event.params.txId });
...
export const processWithdrawal = onDocumentWritten(
  ...
  async (event) => {
    ...
    logger.info('Traitement retrait', { withdrawalId, organizerId, amount });
    ...
    try {
      ...
      logger.info('Retrait traité', { withdrawalId });
    } catch (error) {
...
export const sendPushNotification = onCall(
  ...
  async (request) => {
    ...
    try {
      ...
      logger.info('Notifications envoyées', { successCount: response.successCount });
...
export const sendSmsTicket = onCall(
  ...
  async (request) => {
    ...
    try {
      ...
      logger.info('SMS à envoyer', { phone, ticketCode });
...
export const createDynamicLink = onCall(
  {
    region: 'europe-west1',
    timeoutSeconds: 30,
  },
  async (request) => {
    ...
    try {
      ...
      logger.info('Dynamic link créé', { eventId, shortLink });

      return { success: true, shortLink, longLink: link };
    } catch (error) {
      logger.error('Erreur création dynamic link', error);
